...
サーバー同期はPallaSync 1.0 Core Clientの必須機能です。
...
```text
POST /pallasync/v1/capabilities
```
...
```text
POST /pallasync/v1/chain/create
```
...
```text
POST /pallasync/v1/invite/create
```
...
```text
POST /pallasync/v1/join/request
```
...
```text
POST /pallasync/v1/join/approve
```
...
```text
POST /pallasync/v1/control/pull
```
...
```text
POST /pallasync/v1/control/push
```
...
```text
POST /pallasync/v1/snapshot/push
```
...
```text
POST /pallasync/v1/snapshot/latest
```

### 28.14 Key Package Get

```text
POST /pallasync/v1/key-package/get
```
...
```text
POST /pallasync/v1/peer/signal/send
```

### 28.16 Peer Signal Poll

```text
POST /pallasync/v1/peer/signal/poll
```

### 28.17 Chain Delete

```text
POST /pallasync/v1/chain/delete
```
...
- Data Channel protocolは `pallasync/1`
...
Offer、Answer、ICE CandidateはPallaSync Request Envelopeと同等のDevice署名で保護しなければなりません。
...
```text
ASCII("PALLASYNC-P2P-HELLO-v1\0") ||
...
snapshot_key =
    HKDF-SHA-256(
        IKM = epoch_key,
        salt = chain_id_raw,
        info = ASCII("PALLASYNC-SNAPSHOT-KEY-v1\0") ||
...
サーバーがこれより小さい上限を使用する場合、PallaSync 1.0 Core Server適合を名乗ってはいけません。
...
`protocol_version` は `major.minor` 形式です。

PallaSync 1.0の値は `1.0` です。
...
PallaSync 1.0はトラフィック解析耐性や匿名性を保証しません。
...
```text
chain_id:
Wu7j7vRh3bkeoifgW6iRgZlGRlUq_94tqq_ZqF3YCzQ

device_id:
x1_dmusfFIruHJxawpt6_lpoCC2ECqitU1eJDkfEDjE
```
...
```text
event_key:
qUiT6HjAKtdYRBMym3rwDKahUkBYU80e6Fgh5v7lRV0
...
```json
{"chain_id":"Wu7j7vRh3bkeoifgW6iRgZlGRlUq_94tqq_ZqF3YCzQ","device_id":"x1_dmusfFIruHJxawpt6_lpoCC2ECqitU1eJDkfEDjE","device_seq":1,"event_type":"data","key_epoch":1,"prev_event_hash":null,"protocol_version":"1.0"}
```
...
```text
mYjky0lTLreWMQbiDFqTc8cYtIvNkYsxrUwDs01_2_YVN6Z4JsRx_GS-2cyVQUitd24Nbh7mtqMsA8a4AXihevkIb0ZxcNXLROWczN1uQ_JIAZXiHS_7BCl382iSjrYI5rXJeSDaKVaHjLgdHuyjjaabvWNu8sOwRFZ-6Whyg7CpamdKkJAfAWlVE3c6CVxE5UMb5G3SzezFz6dTq9-5wSIHoT6s6KxiNFbdXSsym8MEVvyyDjdv9CaW3Z1NIRBh4JEBuOGasyqhi3LCJs-8HDEZmUU
```

### 51.7 Signature

```text
1wuJgBf4pabuyPijVpIke14-gzuzDqI5I1k6DqsU6LfED4z9rqN4dlPZ46nO-lcaRhZR1S0zdV33RbUJ8s3RCg
```

### 51.8 Event Hash

```text
uMltT5_PsQZ50WSvvXZ2goQGHrxb9AYjR9d4aVjvIjs
```

## 52. 変更管理

PallaSyncの変更は次の形式で管理します。
...
PallaSync 1.0をProduction Readyとして宣言する前に、最低でも次を完了しなければなりません。
